...
function isPlainObject(val) {
    ...
    return (proto === null ||
        ...
        ((_a = proto === null || proto === void 0 ? void 0 : proto.constructor) === null || _a === void 0 ? void 0 : _a.name) === 'Object');
}
function cloneVal(val, seen) {
    if (seen === void 0) { seen = new WeakSet(); }
    if (val == null || typeof val !== 'object')
        return val;
    if (seen.has(val))
        return val; // circular ref: keep original
    seen.add(val);
    if (Array.isArray(val))
        return val.map(function (v) { return cloneVal(v, seen); });
    if (!isPlainObject(val))
        return val;
    var out = {};
    for (var _i = 0, _a = Object.keys(val); _i < _a.length; _i++) {
        var k = _a[_i];
        out[k] = cloneVal(val[k], seen);
    }
    return out;
}
...
var processCmd = function (data, node, ctx) { return __awaiter(void 0, void 0, void 0, function () {
    var cmd, _a, cmdName, cmdRest, aliasMatch, aliasName, fullCmd, result, nerr, str, literalXmlDelimiter, splitByLineBreak, LINE_BREAK, END_OF_TEXT, START_OF_TEXT, id, relId, pendingDownload, clonedSandbox, _i, _b, k, frozenSandbox_1, curLoop, frozenVars, _c, _d, k, img, pars, html, err_1;
    return __generator(this, function (_e) {
        switch (_e.label) {
            ...
            case 15:
                ...
                for (_i = 0, _b = Object.keys(ctx.jsSandbox || {}); _i < _b.length; _i++) {
                    k = _b[_i];
                    clonedSandbox[k] = cloneVal(ctx.jsSandbox[k]);
                }
                frozenSandbox_1 = __assign(__assign(__assign({}, clonedSandbox), data), ctx.options.additionalJsContext);
                curLoop = (0, reportUtils_1.getCurLoop)(ctx);
                if (curLoop)
                    frozenSandbox_1.$idx = curLoop.idx;
                Object.keys(ctx.vars).forEach(function (varName) {
                    frozenSandbox_1["$".concat(varName)] = cloneVal(ctx.vars[varName]);
                });
                ...
                if (ctx.options.runJs) {
                    frozenVars = {};
                    for (_c = 0, _d = Object.keys(ctx.vars); _c < _d.length; _c++) {
                        k = _d[_c];
                        frozenVars[k] = cloneVal(ctx.vars[k]);
                    }
                    pendingDownload.frozenCtx = __assign(__assign({}, ctx), { vars: frozenVars, loops: ctx.loops.map(function (l) { return (__assign({}, l)); }) });
                }
 ...
 */
var applyImageData = function (ctx, imagePars, pending) {
    var _a;
    ...
    if (imagePars.rotation && pending.xfrmNode) {
        var rot = (imagePars.rotation * 60e3).toString();
    ...
    if (imgData.extension === '.svg') {
        ...
        if (pending.extLstNode) {
            var svgBlipNode = (0, reportUtils_1.newNonTextNode)('asvg:svgBlip', {
                'xmlns:asvg': 'http://schemas.microsoft.com/office/drawing/2016/SVG/main',
                'r:embed': pending.id,
            });
            var svgExtNode = (0, reportUtils_1.newNonTextNode)('a:ext', { uri: '{96DAC541-7B7A-43D3-8B79-37D633B846F1}' }, [svgBlipNode]);
...
function removeDrawingNode(pending) {
    var node = pending.drawingNode;
...
function resolvePendingImages(ctx_1) {
    return __awaiter(this, arguments, void 0, function (ctx, concurrency) {
        var pendingDownloads, errors, limit, results, i, result, pending_1, error, imgError, imagePars, captionBr, captionText, captionT, e_1, imgError;
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    return [4 /*yield*/, Promise.allSettled(pendingDownloads.map(function (pd) {
                            return limit(function () {
                                return (0, jsSandbox_1.runUserJsAndGetRaw)(undefined, pd.code, ctx, pd.frozenSandbox, pd.frozenCtx);
                            });
                        }))];
                case 1:
                    results = _a.sent();
                ...
                case 2:
                    ...
                    removeDrawingNode(pending_1);
                    error = result.reason;
                    imgError = new errors_1.ImageError(error instanceof Error ? error : new Error(String(error)), pending_1.cmd);
                    if (!(ctx.options.errorHandler != null)) return [3 /*break*/, 4];
                    return [4 /*yield*/, ctx.options.errorHandler(imgError, pending_1.cmd)];
                case 3:
                    _a.sent();
                    return [3 /*break*/, 5];
                case 4:
                ...
                case 7:
                    _a.trys.push([7, 8, , 12]);
                    applyImageData(ctx, imagePars, pending_1);
                ...
                case 8:
                    ...
                    if (!(0, errors_1.isError)(e_1))
                        throw e_1;
                    imgError = new errors_1.ImageError(e_1, pending_1.cmd);
...
var processImage = function (ctx, imagePars) {
    ...
    var pending = buildPendingImageNode(ctx, relId, id, '');
    ...
    if (imagePars.caption) {
        ctx.pendingImageNode.caption = [
            (0, reportUtils_1.newNonTextNode)('w:br'),
            (0, reportUtils_1.newNonTextNode)('w:t', {}, [(0, reportUtils_1.newTextNode)(imagePars.caption)]),
        ];
    }
};
