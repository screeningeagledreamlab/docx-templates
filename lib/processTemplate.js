...
exports.resolvePendingImages = exports.splitCommand = exports.getCommand = exports.walkTemplate = exports.findHighestImgId = exports.produceJsReport = exports.extractQuery = exports.newContext = void 0;
...
var DEFAULT_IMAGE_CONCURRENCY = 10;
...
function walkTemplate(data, template, ctx, processor) {
    return __awaiter(this, void 0, void 0, function () {
        var out, nodeIn, nodeOut, move, deltaJump, errors, loopCount, maximumWalkingDepth, curLoop, nextSibling, parent_3, tag, fRemoveNode, buffers, buffers, nodeOutParent, imgNode, captionNodes, parent_4, lastPending, linkNode, parent_5, htmlNode, parent_6, tag, newNode, newNodeTag, parent_7, result, err, innermost_loop, err;
        var _a;
        var _b;
                    ...
                    if (move === 'UP') {
                        ...
                        if (ctx.pendingImageNode &&
                            ...
                            if (parent_4) {
                                ...
                                lastPending = ctx.pendingImageDownloads[ctx.pendingImageDownloads.length - 1];
                                if (lastPending && lastPending.drawingNode === imgNode) {
                                    lastPending.captionParent = parent_4;
                                }
...
var processText = function (data, node, ctx, onCommand) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, cmdDelimiter, failFast, text, segments, outParts, errors, idx, segment, cmdResultText;
...
var processCmd = function (data, node, ctx) { return __awaiter(void 0, void 0, void 0, function () {
    var cmd, _a, cmdName, cmdRest, aliasMatch, aliasName, fullCmd, result, nerr, str, literalXmlDelimiter, splitByLineBreak, LINE_BREAK, END_OF_TEXT, START_OF_TEXT, id, relId, pendingDownload, capturedSandbox_1, isInsideLoop, capturedVars_1, capturedIdx_1, img, pars, html, err_1;
    var _b;
    return __generator(this, function (_c) {
        switch (_c.label) {
            case 0:
            ...
            case 15:
                if (!(cmdName === 'IMAGE')) return [3 /*break*/, 19];
                if (!!(0, reportUtils_1.isLoopExploring)(ctx)) return [3 /*break*/, 18];
                if (!(ctx.options.imageConcurrency != null)) return [3 /*break*/, 16];
                ...
                relId = "img".concat(id);
                pendingDownload = buildPendingImageNode(ctx, relId, id, cmd);
                capturedSandbox_1 = __assign({ __code__: undefined, __result__: undefined }, ctx.jsSandbox);
                isInsideLoop = ctx.loops.length > 0;
                capturedVars_1 = isInsideLoop ? __assign({}, ctx.vars) : null;
                capturedIdx_1 = isInsideLoop ? (_b = (0, reportUtils_1.getCurLoop)(ctx)) === null || _b === void 0 ? void 0 : _b.idx : undefined;
                pendingDownload.fetchImage = function () {
                    // If we captured loop state, temporarily restore it for this evaluation
                    if (capturedVars_1 !== null) {
                        var savedVars = ctx.vars;
                        var savedLoop = (0, reportUtils_1.getCurLoop)(ctx);
                        var savedIdx = savedLoop === null || savedLoop === void 0 ? void 0 : savedLoop.idx;
                        ctx.vars = capturedVars_1;
                        if (savedLoop && capturedIdx_1 !== undefined)
                            savedLoop.idx = capturedIdx_1;
                        try {
                            return (0, jsSandbox_1.runUserJsAndGetRaw)(data, cmdRest, ctx, capturedSandbox_1);
                        }
                        finally {
                            ctx.vars = savedVars;
                ...
                return [3 /*break*/, 18];
            case 16: return [4 /*yield*/, (0, jsSandbox_1.runUserJsAndGetRaw)(data, cmdRest, ctx)];
            case 17:
                img = _c.sent();
                if (img != null) {
                    try {
            ...
            case 19:
                if (!(cmdName === 'LINK')) return [3 /*break*/, 23];
                if (!!(0, reportUtils_1.isLoopExploring)(ctx)) return [3 /*break*/, 22];
                return [4 /*yield*/, (0, jsSandbox_1.runUserJsAndGetRaw)(data, cmdRest, ctx)];
            case 20:
                pars = _c.sent();
                if (!(pars != null)) return [3 /*break*/, 22];
            ...
            case 23:
                if (!(cmdName === 'HTML')) return [3 /*break*/, 27];
                if (!!(0, reportUtils_1.isLoopExploring)(ctx)) return [3 /*break*/, 26];
                return [4 /*yield*/, (0, jsSandbox_1.runUserJsAndGetRaw)(data, cmdRest, ctx)];
            case 24:
                html = _c.sent();
                if (!(html != null)) return [3 /*break*/, 26];
                return [4 /*yield*/, processHtml(ctx, html)];
            case 25:
                _c.sent();
                _c.label = 26;
            case 26: return [3 /*break*/, 28];
            case 27: throw new errors_1.CommandSyntaxError(cmd);
...
var buildPendingImageNode = function (ctx, relId, id, cmd) {
    var node = reportUtils_1.newNonTextNode;
    // Create nodes with placeholder values - will be updated after download resolves
    var extentNode = node('wp:extent', { cx: '0', cy: '0' });
    var picExtNode = node('a:ext', { cx: '0', cy: '0' });
    var xfrmNode = node('a:xfrm', {}, [
        node('a:off', { x: '0', y: '0' }),
        picExtNode,
    ]);
    var docPrNode = node('wp:docPr', { id: id, name: "Picture ".concat(id), descr: '' });
    var cNvPrNode = node('pic:cNvPr', {
        id: '0',
        name: "Picture ".concat(id),
        descr: '',
    });
    var extLstNode = node('a:extLst', {}, [
    ...
    var blipNode = node('a:blip', { 'r:embed': relId, cstate: 'print' }, [
        extLstNode,
    ]);
    ...
    return {
        id: relId,
        fetchImage: function () { return Promise.resolve(undefined); }, // Will be replaced with actual fetch function
        cmd: cmd,
        extentNode: extentNode,
        picExtNode: picExtNode,
        xfrmNode: xfrmNode,
        blipNode: blipNode,
        extLstNode: extLstNode,
        docPrNode: docPrNode,
        cNvPrNode: cNvPrNode,
 ...
 */
function resolvePendingImages(ctx_1) {
    return __awaiter(this, arguments, void 0, function (ctx, concurrency) {
        var pendingDownloads, limit, results, i, result, pending_1, error, imagePars, cx, cy, rot, alt, imgData, thumbnail, thumbId, thumbRelId, svgBlipNode, svgExtNode, captionBr, captionText, captionT, e_1;
        var _a;
        if (concurrency === void 0) { concurrency = DEFAULT_IMAGE_CONCURRENCY; }
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    pendingDownloads = ctx.pendingImageDownloads;
                    if (pendingDownloads.length === 0)
                        return [2 /*return*/];
                    debug_1.logger.debug("Resolving ".concat(pendingDownloads.length, " pending image downloads with concurrency limit of ").concat(concurrency, "..."));
                    limit = (0, p_limit_1.default)(concurrency);
                    return [4 /*yield*/, Promise.allSettled(pendingDownloads.map(function (pd) { return limit(function () { return pd.fetchImage(); }); }))];
                case 1:
                    results = _b.sent();
                    i = 0;
                    _b.label = 2;
                case 2:
                    if (!(i < results.length)) return [3 /*break*/, 13];
                    result = results[i];
                    pending_1 = pendingDownloads[i];
                    if (!(result.status === 'rejected')) return [3 /*break*/, 6];
                    error = result.reason;
                    if (!(ctx.options.errorHandler != null)) return [3 /*break*/, 4];
                    return [4 /*yield*/, ctx.options.errorHandler(error instanceof Error ? error : new Error(String(error)), pending_1.cmd)];
                case 3:
                    _b.sent();
                    return [3 /*break*/, 5];
                case 4:
                    if (ctx.options.failFast) {
                        throw new errors_1.ImageError(error instanceof Error ? error : new Error(String(error)), pending_1.cmd);
                    }
                    _b.label = 5;
                case 5: 
                ...
                case 7:
                    ...
                    if (pending_1.extentNode) {
                        pending_1.extentNode._attrs.cx = cx;
                        pending_1.extentNode._attrs.cy = cy;
                    }
                    if (pending_1.picExtNode) {
                        pending_1.picExtNode._attrs.cx = cx;
                        pending_1.picExtNode._attrs.cy = cy;
                    }
                    // Handle rotation
                    if (imagePars.rotation && pending_1.xfrmNode) {
                        rot = (imagePars.rotation * 60e3).toString();
                        pending_1.xfrmNode._attrs.rot = rot;
                    }
                    alt = imagePars.alt || '';
                    if (pending_1.docPrNode) {
                        pending_1.docPrNode._attrs.descr = alt;
                    }
                    if (pending_1.cNvPrNode) {
                        pending_1.cNvPrNode._attrs.descr = alt;
                    }
                    imgData = getImageData(imagePars);
                    ...
                    if (imgData.extension === '.svg') {
                        ...
                        ctx.images[pending_1.id] = imgData;
                        // Create a new ID for the thumbnail
                        ctx.imageAndShapeIdIncrement += 1;
                        thumbId = String(ctx.imageAndShapeIdIncrement);
                        thumbRelId = "img".concat(thumbId);
                        validateImage(thumbnail);
                        ctx.images[thumbRelId] = thumbnail;
                        // Add SVG extension to extLst
                        if (pending_1.extLstNode) {
                            svgBlipNode = (0, reportUtils_1.newNonTextNode)('asvg:svgBlip', {
                                'xmlns:asvg': 'http://schemas.microsoft.com/office/drawing/2016/SVG/main',
                                'r:embed': pending_1.id,
                            });
                            svgExtNode = (0, reportUtils_1.newNonTextNode)('a:ext', { uri: '{96DAC541-7B7A-43D3-8B79-37D633B846F1}' }, [svgBlipNode]);
                            // Set parent references
                            svgExtNode._parent = pending_1.extLstNode;
                            svgBlipNode._parent = svgExtNode;
                            pending_1.extLstNode._children.push(svgExtNode);
                        }
                        // Update blip to reference thumbnail instead of SVG
                        if (pending_1.blipNode) {
                            pending_1.blipNode._attrs['r:embed'] = thumbRelId;
                        }
                    }
                    else {
                    ...
                    if (imagePars.caption && pending_1.captionParent) {
                        captionBr = (0, reportUtils_1.newNonTextNode)('w:br', {});
                        captionText = (0, reportUtils_1.newTextNode)(imagePars.caption);
                        captionT = (0, reportUtils_1.newNonTextNode)('w:t', {}, [captionText]);
                        // Set parent references
                        captionBr._parent = pending_1.captionParent;
                        captionT._parent = pending_1.captionParent;
                        captionText._parent = captionT;
                        // Add caption nodes after the drawing node
                        pending_1.captionParent._children.push(captionBr);
                        pending_1.captionParent._children.push(captionT);
                    }
                    return [3 /*break*/, 12];
                case 8:
                    e_1 = _b.sent();
                    ...
                    return [4 /*yield*/, ctx.options.errorHandler(e_1, pending_1.cmd)];
                case 9:
                    _b.sent();
                    return [3 /*break*/, 11];
                case 10:
                    if (ctx.options.failFast) {
                        throw new errors_1.ImageError(e_1, pending_1.cmd);
                    }
                    _b.label = 11;
                ...
                case 13:
                    ...
                    debug_1.logger.debug('All pending image downloads resolved.');
                    return [2 /*return*/];
            }
        });
    });
}
exports.resolvePendingImages = resolvePendingImages;
...
var processImage = function (ctx, imagePars, preAssignedRelId) {
    var _a;
    validateImagePars(imagePars);
    ...
    if (preAssignedRelId) {
        ...
        var imgData = getImageData(imagePars);
        validateImage(imgData);
